...
let trxs = "";
try {
    trxs = await viz.auth.signTransaction(trx, {"active": conf.active_key});
} catch (error) {
...
let trxs = "";
try {
trxs = await viz.auth.signTransaction(trx, {"posting": conf.posting_key});
} catch (error) {
console.log("Не удалось подписать транзакцию: " + error.message);
}
try {
...
json.contractPayload = data;
    return await viz.broadcast.customAsync(conf.posting_key, [], [conf.login], 'loser-game', JSON.stringify(json));
}
